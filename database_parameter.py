...
class GenericTableValidator:
    ...
    def validate_tables(self, database: str, table1_name: str, table2_name: str,
        ...
        """
        Enhanced table validation with database parameter and smart sampling based on column specification
        
        Args:
            database: Database name to prepend to table names
            table1_name: Name of first table (can be schema.table or just table)
            table2_name: Name of second table (can be schema.table or just table)
        ...
        self.logger.info(f"Original table1_name: {table1_name} -> Full name: {full_table1_name}")
        self.logger.info(f"Original table2_name: {table2_name} -> Full name: {full_table2_name}")
        ...
        if target_schema is None:
            # Extract schema from full_table1_name (e.g., "DB.SCHEMA.TABLE" -> "DB.SCHEMA")
            table_parts = full_table1_name.split('.')
            if len(table_parts) >= 2:
                target_schema = '.'.join(table_parts[:-1])
            else:
                target_schema = f"{database}.PUBLIC" if database else "PUBLIC"  # Default fallback
        
        self.logger.info("="*80)
        self.logger.info("STARTING MEMORY-OPTIMIZED TABLE VALIDATION")
        self.logger.info("="*80)
        self.logger.info(f"Database: {database}")
        self.logger.info(f"Table 1 ({table1_alias}): {full_table1_name}")
        self.logger.info(f"Table 2 ({table2_alias}): {full_table2_name}")
        ...
        try:
            ...
            # Run memory-optimized comparison using full table names
            mismatched_rows, table1_only_rows, table2_only_rows, summary_stats = self.compare_tables_memory_optimized(
                full_table1_name, full_table2_name, pk_columns, comp_columns, table1_alias, table2_alias, final_max_sample_size, user_specified_columns
            )
    ...
    def print_sample_mismatches(self, database: str, table1_name: str, table2_name: str,
                               primary_key_columns: Optional[List[str]] = None,
                               comparison_columns: Optional[List[str]] = None,
                               table1_alias: str = "SQL", 
                               table2_alias: str = "MDP",
        ...
        """
        Print sample mismatches for verification with column filtering and database parameter
        ...
        print(f"Table 1: {full_table1_name}")
        print(f"Table 2: {full_table2_name}")
        ...
        try:
            ...
            # Create temporary views using full table names
            table1_df = self.session.table(full_table1_name)
            table2_df = self.session.table(full_table2_name)
    ...
    def print_validation_report(self, results: Dict[str, Any]):
        ...
        print(f"Database: {results.get('database', 'N/A')}")
        ...
        print("1. Verify: validator.print_sample_mismatches(database, table1, table2, pk_cols, comp_cols, alias1, alias2, 10)")
...
def validate_tables_sp(session: Session, 
                      ...
                      target_schema: str = None,
                      max_sample_size = None,
                      max_auto_columns: int = None):
    """
    Stored procedure version with database parameter and smart sampling logic
    ...
    try:
        ...
        # Run memory-optimized validation with database parameter
...
def main():
    """
    Main function for testing the memory-optimized table validator with database parameter
    """
    try:
        validator = GenericTableValidator()
        
        # Example usage with database parameter
        database = 'BCBSND_CONFORMED_DEV'
        table1 = 'OUTBOUND.MEMBER_ENROLLMENT_MASTER'
        table2 = 'OUTBOUND.MEMBER_ENROLLMENT_MASTER_PROD'
        
        # Sample mismatches with enhanced column control and database parameter
