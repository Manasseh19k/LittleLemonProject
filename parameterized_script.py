...
def main():
    ...
    try:
        ...
        results = validator.validate_tables(
            ...
            primary_key_columns=['MEMBER_ID'],  # Specify primary key columns
            table1_alias='SQL',  # Dev environment alias
            table2_alias='MDP',  # Prod environment alias
...
# Additional utility functions for the stored procedure
def create_validation_stored_procedure_sql() -> str:
    """
    Generate the SQL to create the stored procedure in Snowflake
    
    Returns:
        SQL string to create the stored procedure
    """
    return """
...
COMMENT = 'Comprehensive table validation stored procedure for comparing any two tables row by row across all columns'
AS
$
# Paste the entire GenericTableValidator class and validate_tables_sp function here
# This stored procedure can be used by the entire team for table validation
...
# [Include the complete GenericTableValidator class from above]
# [Include the validate_tables_sp function from above]
...
def get_usage_examples() -> str:
    """
    Get usage examples for the team
...
1. Basic Usage (Auto-detect primary keys):
   CALL VALIDATE_TABLES_SP(
       'DATABASE.SCHEMA.DEV_TABLE',
       'DATABASE.SCHEMA.PROD_TABLE'
   );

2. With Specific Primary Keys:
   CALL VALIDATE_TABLES_SP(
       'DATABASE.SCHEMA.DEV_TABLE',
       'DATABASE.SCHEMA.PROD_TABLE',
       'MEMBER_ID,ACCOUNT_ID'
...
- TABLE_VALIDATION_MISMATCHES_[ALIAS1]_VS_[ALIAS2]_[TIMESTAMP]
- TABLE_VALIDATION_[ALIAS1]_ONLY_[TIMESTAMP]  
- TABLE_VALIDATION_[ALIAS2]_ONLY_[TIMESTAMP]

=== STAGE FILES CREATED ===
- @stage/table_validation_mismatches_[alias1]_vs_[alias2]_[timestamp].csv
- @stage/table_validation_[alias1]_only_[timestamp].csv
- @stage/table_validation_[alias2]_only_[timestamp].csv
- @stage/table_validation_summary_[alias1]_vs_[alias2]_[timestamp].csv

=== KEY FEATURES ===
✅ Compares ALL columns in both tables (not just specific ones)
✅ Auto-detects primary key columns or uses specified ones
✅ Handles any table names as parameters
✅ Supports custom aliases (SQL/MDP, DEV/PROD, etc.)
✅ Works across different environments
✅ Row-by-row comprehensive comparison
✅ Creates detailed mismatch reports
✅ Identifies rows unique to each table
✅ Generates summary statistics
✅ Creates both tables and CSV files for results
"""
...
if __name__ == "__main__":
    ...
    # Print usage examples for the team
    print("\n" + "="*80)
    print("STORED PROCEDURE SETUP INSTRUCTIONS")
    print("="*80)
    print("1. Copy the complete script to create the stored procedure")
    print("2. Execute the CREATE OR REPLACE PROCEDURE statement in Snowflake")
    print("3. Use the examples below for team validation needs")
